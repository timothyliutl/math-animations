...
class Spaghetti_2(ThreeDScene):
    def construct(self):
        ...
        for i in range(-4,4):
            self.play(e.animate.set_value(i))
            if i<0:
                curve1 = ParametricFunction(lambda t: axes.c2p(*(parametric_curves_z_neg(i, t)[0])), t_range=[-4,4])
                curve2 = ParametricFunction(lambda t: axes.c2p(*(parametric_curves_z_neg(i, t)[1])), t_range=[-4,4])
            if i>=0:
                curve1 = ParametricFunction(lambda t: axes.c2p(*(parametric_curves_z_pos(i, t)[0])), t_range=[-4,4])
                curve2 = ParametricFunction(lambda t: axes.c2p(*(parametric_curves_z_pos(i, t)[1])), t_range=[-4,4])
            self.add_fixed_in_frame_mobjects(decimal)
